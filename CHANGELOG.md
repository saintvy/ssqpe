...
- Row counts below one thousand now use one decimal place, with the decimal portion visually de-emphasized in the tree, graph cards, and operator details.
...
- The Time view now shows statement-level `WaitStats` before the operator tree, with independently normalized time and `WaitCount` scales and localized explanations for common wait types.

## [0.2.11] - 2026-08-03

### Added

- Adaptive Join details now show the actual join type selected at runtime from `RunTimeCountersPerThread.ActualJoinType`.
