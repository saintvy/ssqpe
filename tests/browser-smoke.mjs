...
<StmtSimple StatementId="1" StatementType="SELECT" StatementText="synthetic repeated subtree"><QueryPlan CompileTime="23">
<MemoryGrantInfo MaxUsedMemory="2048"/><QueryTimeStats CpuTime="93600" ElapsedTime="351000"/>
<RelOp NodeId="0" PhysicalOp="Concatenation" LogicalOp="Concatenation" EstimateRows="2" EstimatedTotalSubtreeCost="2"><OutputList><ColumnReference Database="[db]" Schema="[dbo]" Table="[A]" Column="Id"/><ColumnReference Database="[db]" Schema="[dbo]" Table="[A]" Column="Name"/></OutputList><DefinedValues><DefinedValue><ColumnReference Column="Expr1000"/></DefinedValue></DefinedValues><Predicate><ScalarOperator ScalarString="[A].[Id]&gt;(0) OR [A].[Id]&lt;(100)"/></Predicate><RunTimeInformation><RunTimeCountersPerThread Thread="0" ActualRows="2" ActualExecutions="1" ActualElapsedms="351000" ActualCPUms="93600"/></RunTimeInformation>
<Concatenation>
...
<RelOp NodeId="1" PhysicalOp="Index Seek" LogicalOp="Index Seek" EstimateRows="1" EstimatedRowsRead="1" EstimatedTotalSubtreeCost="0.4"><RunTimeInformation><RunTimeCountersPerThread Thread="0" ActualRows="39553" ActualRowsRead="39553" ActualExecutions="39553"/></RunTimeInformation><IndexScan/></RelOp>
<RelOp NodeId="2" PhysicalOp="Index Seek" LogicalOp="Index Seek" EstimateRows="2" EstimatedRowsRead="8" EstimatedTotalSubtreeCost="0.4"><RunTimeInformation><RunTimeCountersPerThread Thread="0" ActualRows="20" ActualRowsRead="20" ActualExecutions="10"/><RunTimeCountersPerThread Thread="1" ActualRows="60" ActualRowsRead="60" ActualExecutions="10"/></RunTimeInformation><IndexScan/></RelOp>
</Concatenation></RelOp></QueryPlan></StmtSimple></Statements></Batch></BatchSequence></ShowPlanXML>`;
...
try {
  ...
  if (structure.version!=='v0.3.4' || structure.versionFont>11 || structure.versionColor==='rgb(237, 241, 247)') failures.push(`Application version label failed: ${JSON.stringify(structure)}`);
  ...
  const planRename = await evaluate(`(async()=>{
    const button=document.getElementById('renamePlanBtn'),input=document.getElementById('planTitleInput'),title=document.getElementById('planTitle');
    button.click();const started=!input.classList.contains('hidden')&&document.activeElement===input&&input.selectionStart===0&&input.selectionEnd===input.value.length;
    input.value='Cancelled name';input.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}));
    const cancelled=title.textContent==='Smoke custom name'&&input.classList.contains('hidden');
    button.click();input.value='Renamed smoke plan';input.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}));
    await new Promise(resolve=>setTimeout(resolve,100));
    const historyNames=Array.from(document.querySelectorAll('#homeHistoryList .history-name')).map(item=>item.textContent);
    return {started,cancelled,title:title.textContent,inputHidden:input.classList.contains('hidden'),buttonVisible:!button.classList.contains('hidden'),historyNames,toast:document.getElementById('toast').textContent};
  })()`);
  if (!planRename.started || !planRename.cancelled || planRename.title!=='Renamed smoke plan' || !planRename.inputHidden || !planRename.buttonVisible || !planRename.historyNames.includes('Renamed smoke plan') || planRename.toast!=='Plan renamed') failures.push(`Inline plan rename failed: ${JSON.stringify(planRename)}`);
  ...
  const treePoint = await evaluate(`(() => {const tree=document.getElementById('tree'),name=tree.querySelector('.tree-name');tree.scrollLeft=0;const r=tree.getBoundingClientRect(),n=name.getBoundingClientRect();return {x:n.left+Math.min(70,n.width/2),y:n.top+n.height/2,left:r.left+10,canPan:tree.scrollWidth>tree.clientWidth}})()`);
  if (treePoint.canPan) {
    await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:treePoint.x,y:treePoint.y,button:'none'});
    await send('Input.dispatchMouseEvent',{type:'mousePressed',x:treePoint.x,y:treePoint.y,button:'left',clickCount:1});
    await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:treePoint.x-15,y:treePoint.y,button:'left',buttons:1});
  ...
  const detailCopy = await evaluate(`(async() => {
    Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async value=>{window.__detailCopied=value}}});
    const findSection=label=>Array.from(document.querySelectorAll('#drawerBody .detail-section')).find(section=>{const heading=section.querySelector('.detail-heading h3,summary span');return heading?.textContent.startsWith(label)});
    const copy=async label=>{const section=findSection(label),button=section?.querySelector('.detail-copy-button'),wasOpen=section?.open;window.__detailCopied=null;button?.click();await new Promise(resolve=>setTimeout(resolve,0));return {value:window.__detailCopied,button:Boolean(button),stayedClosed:section?.tagName!=='DETAILS'||(!wasOpen&&!section.open)};};
    const primary=await copy('Primary metrics'),io=await copy('I/O and cache'),predicates=await copy('Operands and predicates'),outputs=await copy('Output columns'),estimates=await copy('Optimizer estimates');
    const buttons=Array.from(document.querySelectorAll('#drawerBody .detail-copy-button')),rect=buttons[0]?.getBoundingClientRect();
    return {primary,io,predicates,outputs,estimates,buttons:buttons.length,allIcons:buttons.every(button=>Boolean(button.querySelector('svg'))),width:rect?.width,height:rect?.height,allLabels:buttons.every(button=>button.title==='Copy section'&&button.getAttribute('aria-label')==='Copy section'),allRightAligned:buttons.every(button=>Math.abs(button.getBoundingClientRect().right-button.closest('.detail-section').getBoundingClientRect().right)<1)};
  })()`);
  const primaryTsv=detailCopy.primary.value?.split('\n')||[],outputLines=detailCopy.outputs.value?.split('\n')||[];
  if (!detailCopy.primary.button || primaryTsv.length<5 || !primaryTsv.every(line=>line.includes('\t')) || !detailCopy.io.value?.includes('Logical reads\t0') || detailCopy.predicates.value!=='[A].[Id]>(0) OR [A].[Id]<(100)' || detailCopy.predicates.value.includes('\nOR') || !detailCopy.predicates.stayedClosed || outputLines.length!==2 || outputLines.some(line=>line.includes('\t')) || !detailCopy.outputs.stayedClosed || !detailCopy.estimates.value?.includes('Estimate CPU\t0') || detailCopy.buttons<6 || !detailCopy.allIcons || Math.abs(detailCopy.width-22)>1 || Math.abs(detailCopy.height-22)>1 || !detailCopy.allLabels || !detailCopy.allRightAligned) failures.push(`Detail-section copy controls failed: ${JSON.stringify(detailCopy)}`);
  ...
  const cameraBounds = await evaluate(`new Promise(resolve => {
    const canvas=document.getElementById('canvasWrap'),stage=document.getElementById('graphStage'),graph=document.getElementById('graph'),drawer=document.getElementById('detailsDrawer');
    const sample=()=>{const scale=graph.getBoundingClientRect().width/graph.offsetWidth,inset=drawer.classList.contains('open')?drawer.offsetWidth:0,baseWidth=Math.max(graph.offsetWidth,graph.scrollWidth),expectedStage=Math.max(canvas.clientWidth,Math.ceil(baseWidth*scale+inset+24)),expectedMax=Math.max(0,expectedStage-canvas.clientWidth);canvas.scrollLeft=1e9;const max=canvas.scrollLeft,rightEdge=baseWidth*scale-max;return {scale,inset,stage:stage.scrollWidth,expectedStage,max,expectedMax,rightEdge,availableRight:canvas.clientWidth-inset};};
    requestAnimationFrame(()=>{const open=sample();document.getElementById('closeDetails').click();canvas.dispatchEvent(new WheelEvent('wheel',{deltaY:10000,ctrlKey:true,clientX:100,clientY:100,cancelable:true,bubbles:true}));requestAnimationFrame(()=>requestAnimationFrame(()=>resolve({open,closed:sample()})));});
  })`);
  if (cameraBounds.open.inset<=0 || Math.abs(cameraBounds.open.stage-cameraBounds.open.expectedStage)>2 || Math.abs(cameraBounds.open.max-cameraBounds.open.expectedMax)>2 || (cameraBounds.open.expectedMax>0 && cameraBounds.open.rightEdge>cameraBounds.open.availableRight-20) || cameraBounds.closed.inset!==0 || Math.abs(cameraBounds.closed.scale-.35)>.01 || Math.abs(cameraBounds.closed.stage-cameraBounds.closed.expectedStage)>2 || Math.abs(cameraBounds.closed.max-cameraBounds.closed.expectedMax)>2 || cameraBounds.closed.rightEdge<=0) failures.push(`Zoom-aware graph camera bounds failed: ${JSON.stringify(cameraBounds)}`);
  ...
  const rowsPerExecution = await evaluate(`(() => {
    ...
    const decimals=Array.from(document.querySelectorAll('.row-decimal')),opacity=decimals.length?parseFloat(getComputedStyle(decimals[0]).opacity):1;
    const stat=key=>Array.from(document.querySelectorAll('[data-row-stat="'+key+'"] .wait-scale-value')).map(item=>item.textContent);
    const statsTree=document.querySelector('.rows-stats-tree'),firstOperator=document.querySelector('.tree-row');
    return {treeOne:row('1').querySelector('.tree-value').textContent,treeParallel:row('2').querySelector('.tree-value').textContent,graphOne:graphValue('1'),graphParallel:graphValue('2'),nodeOneAlert:Boolean(card('1').querySelector('[data-alert="rows"]')),estimated:details['Estimated rows per execution'],actualPerExecution:details['Actual rows per execution'],actualAllExecutions:details['Actual rows for all executions'],executions:details.Executions,treeDecimals:row('1').querySelectorAll('.row-decimal').length,graphDecimals:card('1').querySelectorAll('.node-stats .row-decimal').length,detailDecimals:document.querySelectorAll('#drawerBody .row-decimal').length,decimalText:decimals[0]?.textContent,opacity,statsCount:statsTree.querySelectorAll('[data-row-stat]').length,statsBeforeOperators:Boolean(statsTree.compareDocumentPosition(firstOperator)&Node.DOCUMENT_POSITION_FOLLOWING),statsTitle:statsTree.querySelector('.wait-heading .wait-name').textContent,output:stat('output'),processed:stat('processed'),read:stat('read')};
  })()`);
  if (rowsPerExecution.treeOne!=='1.0 / 1.0' || rowsPerExecution.treeParallel!=='2.0 / 4.0' || rowsPerExecution.graphOne!=='1.0 / 1.0' || rowsPerExecution.graphParallel!=='2.0 / 4.0' || rowsPerExecution.nodeOneAlert || rowsPerExecution.estimated!=='1.0' || rowsPerExecution.actualPerExecution!=='1.0' || rowsPerExecution.actualAllExecutions!=='39.55K' || rowsPerExecution.executions!=='39.55K' || rowsPerExecution.treeDecimals!==2 || rowsPerExecution.graphDecimals!==2 || rowsPerExecution.detailDecimals<2 || rowsPerExecution.decimalText!=='.0' || rowsPerExecution.opacity>=0.5 || rowsPerExecution.statsCount!==3 || !rowsPerExecution.statsBeforeOperators || rowsPerExecution.statsTitle!=='Stats' || JSON.stringify(rowsPerExecution.output)!==JSON.stringify(['2.0','2.0']) || JSON.stringify(rowsPerExecution.processed)!==JSON.stringify(['5.0','7.0']) || JSON.stringify(rowsPerExecution.read)!==JSON.stringify(['9.0','5.0'])) failures.push(`Rows statistics, formatting, or normalization failed: ${JSON.stringify(rowsPerExecution)}`);
  ...
  const memoryMetric = await evaluate(`(() => {
    ...
    const result={label:button.textContent,active:button.classList.contains('active'),root:row('0'),child:row('1'),legend:document.getElementById('metricNotice').textContent,infoTitle:infoTree.querySelector('.wait-heading .wait-name').textContent,infoCount:infoTree.querySelectorAll('[data-memory-info]').length,infoBeforeOperators:Boolean(infoTree.compareDocumentPosition(firstOperator)&Node.DOCUMENT_POSITION_FOLLOWING),grantedInfo:info('GrantedMemory'),usedInfo:info('MaxUsedMemory'),grantInMemory:Boolean(infoTree.querySelector('[data-memory-info="GrantWaitTime"]'))};
    document.querySelector('[data-metric="time"]').click();const grant=document.querySelector('[data-time-stat="GrantWaitTime"]');return {...result,grantInTime:Boolean(grant),grantValue:grant?.querySelector('.wait-scale-value').textContent,grantParent:grant?.previousElementSibling?.querySelector('.wait-name').textContent};
  })()`);
  if (memoryMetric.label!=='Memory' || !memoryMetric.active || Math.abs(memoryMetric.root.granted-100)>0.1 || Math.abs(memoryMetric.root.used-50)>0.1 || memoryMetric.root.value!=='100 / 25' || !memoryMetric.root.title.includes('2.00 MB') || !memoryMetric.root.title.includes('1.00 MB') || Math.abs(memoryMetric.child.granted-100)>0.1 || Math.abs(memoryMetric.child.used-25)>0.1 || memoryMetric.child.value!=='50 / 10' || !memoryMetric.legend.includes('Granted memory') || !memoryMetric.legend.includes('tempdb reads / writes') || memoryMetric.infoTitle!=='MemoryGrantInfo' || memoryMetric.infoCount!==4 || !memoryMetric.infoBeforeOperators || memoryMetric.grantedInfo.value!=='4.00 MB' || Math.abs(memoryMetric.grantedInfo.width-100)>0.1 || memoryMetric.usedInfo.value!=='2.00 MB' || Math.abs(memoryMetric.usedInfo.width-50)>0.1 || memoryMetric.grantInMemory || !memoryMetric.grantInTime || memoryMetric.grantValue!=='12 ms' || memoryMetric.grantParent!=='MemoryGrantInfo') failures.push(`Memory metric scale, grant wait placement, or tempdb counters failed: ${JSON.stringify(memoryMetric)}`);
