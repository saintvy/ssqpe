...
try {
  ...
  if (structure.version!=='v0.3.3' || structure.versionFont>11 || structure.versionColor==='rgb(237, 241, 247)') failures.push(`Application version label failed: ${JSON.stringify(structure)}`);
  ...
  const rowsPerExecution = await evaluate(`(() => {
    ...
    return {treeOne:row('1').querySelector('.tree-value').textContent,treeParallel:row('2').querySelector('.tree-value').textContent,graphOne:graphValue('1'),graphParallel:graphValue('2'),nodeOneAlert:Boolean(card('1').querySelector('[data-alert="rows"]')),estimated:details['Estimated rows per execution'],actualPerExecution:details['Actual rows per execution'],actualAllExecutions:details['Actual rows for all executions'],executions:details.Executions,treeDecimals:row('1').querySelectorAll('.row-decimal').length,graphDecimals:card('1').querySelectorAll('.node-stats .row-decimal').length,detailDecimals:document.querySelectorAll('#drawerBody .row-decimal').length,decimalText:decimals[0]?.textContent,opacity};
  })()`);
  if (rowsPerExecution.treeOne!=='1.0 / 1.0' || rowsPerExecution.treeParallel!=='2.0 / 4.0' || rowsPerExecution.graphOne!=='1.0 / 1.0' || rowsPerExecution.graphParallel!=='2.0 / 4.0' || rowsPerExecution.nodeOneAlert || rowsPerExecution.estimated!=='1.0' || rowsPerExecution.actualPerExecution!=='1.0' || rowsPerExecution.actualAllExecutions!=='39.55K' || rowsPerExecution.executions!=='39.55K' || rowsPerExecution.treeDecimals!==2 || rowsPerExecution.graphDecimals!==2 || rowsPerExecution.detailDecimals<2 || rowsPerExecution.decimalText!=='.0' || rowsPerExecution.opacity>=0.5) failures.push(`Rows per execution formatting or normalization failed: ${JSON.stringify(rowsPerExecution)}`);
